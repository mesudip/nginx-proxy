...
class CloudflareTunnelManager:
    ...
    def get_config(self):
        api_url = f"{self.cloudflare.api_base_url}/accounts/{self.account_id}/cfd_tunnel/{self.tunnel_id}/configurations"
        response = self.cloudflare._get(api_url, step="Cloudflare Get Tunnel Config")
        return response.json()["result"]["config"]

    def update_config(self, config):
        api_url = f"{self.cloudflare.api_base_url}/accounts/{self.account_id}/cfd_tunnel/{self.tunnel_id}/configurations"
        return self.cloudflare._req_with_retry("PUT", api_url, json_data={"config":config},step="Cloudflare Update Tunnel Config")
    ...
    def _list_dns_records(self, domain: str, type: str = None, name: str = None) -> list:
        ...
        api_url = f"{self.cloudflare.api_base_url}/zones/{zone_id}/dns_records" + (f"?{query_string}" if query_string else "")
    ...
    def _get_tunnel_id_by_hostname(self, hostname: str):
        records = self._list_dns_records(domain=hostname, type="CNAME", name=hostname)
        for record in records:
            if record['name'] == hostname and record['type'] == 'CNAME' and record['content'].endswith('.cfargotunnel.com'):
