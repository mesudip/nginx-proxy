            ...
            self.client._emit_event(
                {
                    ...
                    "Type": "container",
                    "Action": "create",
                    "Actor": {"ID": cid, "Attributes": {"name": name, "image": image}},
                    "scope": "local",
                    "time": int(time.time()),
...
class MockImage:
    def __init__(self, id, tag, client):
        ...
        self.client: "DockerTestClient" = client
        self.attrs = {"Id": id, "RepoTags": self.tags}
                ...
                self.client._emit_event(
                    {
                        ...
                        "Actor": {
                            "ID": self.id,
                            "Attributes": {"name": self.name, "image": self.image, "exitCode": "0"},  # Mocked
                        },
                        "scope": "local",
                        "time": int(time.time()),
                        "timeNano": time.time_ns(),
    ...
    def remove(self, **kwargs):
        ...
        with self.client._lock:
            ...
            if self.status == "running" and not force:
                raise ValueError(
                    f"You cannot remove a running container {self.name}. Stop the container before attempting removal or force=True"
                )
            ...
            self.client._emit_event(
                {
                    "Type": "network",
                    "Action": "create",
                    "Actor": {"ID": nid, "Attributes": {"name": name, "type": "bridge"}},  # Mocked as bridge
                    "scope": "local",
                    "time": int(time.time()),
                    "timeNano": time.time_ns(),
                }
                ...
                self.client._emit_event(
                    {
                        "Type": "network",
                        "Action": "destroy",
                        "Actor": {"ID": self.id, "Attributes": {"name": self.name, "type": "bridge"}},
                        "scope": "local",
                        "time": int(time.time()),
                        "timeNano": time.time_ns(),
                    }
            ...
            self.client._emit_event(
                {
                    "Type": "network",
                    "Action": "connect",
                    "Actor": {"ID": self.id, "Attributes": {"container": cont_id, "name": self.name, "type": "bridge"}},
                    "scope": "local",
                    "time": int(time.time()),
                    "timeNano": time.time_ns(),
                }
