...
def expect_server_up_integration(nginx_proxy_container: docker.models.containers.Container, server_name: str, exact=True, timeout=10):
    """
    ...
    pytest.fail(f"Server for '{server_name}' not found or not up after {timeout} seconds. Current config:\n{all_servers_str}")

def expect_server_down_integration(nginx_proxy_container: docker.models.containers.Container, server_name: str, timeout=10):
    """
...
def expect_server_not_present_integration(nginx_proxy_container: docker.models.containers.Container, server_name: str, timeout=10):
    """
...
def test_webserver_add_container_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
...
def test_webserver_remove_container_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
...
def test_webserver_add_network_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
...
def test_webserver_remove_network_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
...
def test_webserver_recreate_same_name_container_with_different_host_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
    ...
    try:
        ...
        expect_server_down_integration(nginx_proxy_container[0], old_virtual_host, timeout=15) # Ensure old is still down
    finally:
        if backend_new:
            backend_new.remove(force=True)

def test_webserver_add_container_with_ssl_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
...
def test_webserver_add_two_containers_with_same_virtual_host_integration(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
    ...
    try:
        ...
        server = expect_server_up_integration(nginx_proxy_container[0], virtual_host, timeout=15) # Already waited, so quick check
...
def test_when_container_shuts_down__then_ip_removed_from_upstream(nginx_proxy_container: docker.models.containers.Container, docker_client: docker.DockerClient, test_network: docker.models.networks.Network):
    """
    Test that adding two containers with the same VIRTUAL_HOST creates an upstream block
    and the server block uses it.
    """
    ...
    try:
        ...
        upstream = next((u for u in config.upstreams if virtual_host in u.parameters), None)
        assert upstream is not None, f"Upstream block for {virtual_host} not found. Config:\n{config_str}"
        assert len(upstream.get_directives('server')) == 3, f"Expected 3 servers in upstream, found {len(upstream.get_directives('server'))}. Config:\n{config_str}"
        ...
        config = HttpBlock.parse(config_str)   
        upstream = next((u for u in config.upstreams if virtual_host in u.parameters), None)
        assert upstream is None or len(upstream.get_directives('server')) == 1, f"Expected 1 server in upstream after one shutdown, found {len(upstream.get_directives('server'))}. Config:\n{config_str}"
        expect_server_up_integration(nginx_proxy_container[0], virtual_host, timeout=15) # Already waited, so quick check
