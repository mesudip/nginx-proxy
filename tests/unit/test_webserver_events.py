...
def expect_server_down(nginx: DummyNginx, server_name: str):
    ...
    assert len(locations) == 0, f"Server for {server_name} should have no locations (except acme). All servers:\n{all_servers_str}"
...
def test_webserver_add_container(docker_client:DockerTestClient,nginx:DummyNginx):
    ...
    container = docker_client.containers.run( "nginx:alpine",name=container_name, 
                                 environment=env,
                                 network="frontend")
