...
if "PYTHON_DEBUG_ENABLE" in os.environ:
    if os.environ["PYTHON_DEBUG_ENABLE"].strip() == "true":
        if "host" not in config:
            config["host"]=re.findall("([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)+",subprocess.run(["ip","route"],stdout=subprocess.PIPE).stdout.decode().split("\n")[0])[0]
...
def process_network_event(action, event):
    if action == "create":
        ...
        if action == "disconnect":
            ...
            hosts.connect(network=event["Actor"]["ID"], container=event["Actor"]["Attributes"]["container"],scope=event["scope"])
    elif action == "destroy":
